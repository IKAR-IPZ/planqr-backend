...
INSERT INTO tablet_priority_message_presets (
    id,
    name,
    template_id,
    priority,
...
SELECT 'preset-evacuation', 'Ewakuacja', 'evac', 10, 'end_of_day', 0, 1, NOW(), NOW()
WHERE EXISTS (
    SELECT 1 FROM tablet_priority_message_templates WHERE id = 'evac'
)
AND NOT EXISTS (
    SELECT 1 FROM tablet_priority_message_preset_seed_state WHERE id = 1
)
...
SELECT 'preset-rectors-day', 'Dzień rektorski', 'dzien_rektorski', 7, 'tomorrow', 0, 1, NOW(), NOW()
WHERE EXISTS (
    SELECT 1 FROM tablet_priority_message_templates WHERE id = 'dzien_rektorski'
)
AND NOT EXISTS (
    SELECT 1 FROM tablet_priority_message_preset_seed_state WHERE id = 1
)
...
INSERT INTO tablet_priority_message_preset_seed_state (id, seeded_at)
VALUES (1, NOW())
ON CONFLICT (id) DO NOTHING;
